window.onload = () => {
  ...
  //   // console.log(res)
  //   //TODO: Find a way to get the last time it was scraped. and if old, fetch it.
  //   buildWebpage(res)
  // } else {
  //   console.log("res is true")
  //   fetchURL(url)
...
function buildWebpage(res) {
  console.log("res in build:", res)
  res = JSON.parse(res)
  ...
  try {
    fetch(url).then((response) => {
      Promise
        ...
        .then((res) => {
          localStorage.setItem("contendersScheduleObject", res)
          // res = JSON.parse(res)
          buildWebpage(res)
        })
    }).catch((err) => {
